...
const AddSubCategoryModal = ({ isOpen, onClose, categories = [], onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(
        isAuthError ? 'Please login' : rawMsg || 'Failed to create subcategory'
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4">
      <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl relative animate-in fade-in zoom-in duration-200">
        <div className="flex items-center justify-between mb-4 border-b border-slate-100 pb-3">
          <h3 className="text-lg font-bold text-[#003b5c]">Add Subcategory</h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            ...
            <select
              ...
              className="w-full py-2.5 px-3 border border-slate-300 rounded-lg text-sm text-slate-800 bg-white focus:outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20"
              required
            >
          ...
          <div className="flex justify-end gap-3 mt-6">
            ...
            >
              {loading ? <LoadingSpinner size="sm" /> : 'Create Subcategory'}
            </button>
          </div>
        </form>
      </div>
